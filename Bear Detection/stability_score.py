...
tracker = DeepSort(max_age=30, nn_budget=70, max_iou_distance=0.5)

# Constants
FRAME_SKIP = 2  
OUTPUT_VIDEO_PATH = "bear_nightcam_OP2.mp4"
...
for cls_id, stability_score in object_stability_scores.items():
    if (stability_score > STABILITY_THRESHOLD and cls_id in WILD_ANIMALS) or \
       (cls_id == 21 and object_persistence[cls_id] >= MIN_FRAME_PERSISTENCE and np.mean(object_confidences[cls_id]) > 0.5):
